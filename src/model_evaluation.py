...
from src.focal_loss import FraudDetectionNN
...
def evaluate_FraudDetectionNN_focal_loss(config):
    ...
    eval_classification_report_confusion_matrix(
        y_true=y_test, y_pred=X_test_pred, title="FraudDetectionNN  test"
    )
    print(
        f"Precision Recall - AUC: {eval_auc_precision_recall_curve(y_pred_prob=X_test_pred_prob, y_true=y_test)}"
    )


def evaluate_VotingClassifier(config, model_path):
    model_path = (
        "models/2024_07_18_07_29/trained_models.pkl"  # path to voting classifier model
    )
    ...
    eval_classification_report_confusion_matrix(
        y_pred=x_test_pred, y_true=y_test, title="Voting Classifier test"
    )
    print(
        f"Precision Recall - AUC: {eval_auc_precision_recall_curve(y_pred_prob=x_test_pred_prob, y_true=y_test)}"
    )
...
if __name__ == "__main__":
    ...
    evaluate_VotingClassifier(
        config=config, model_path=model_path
    )  # evaluate voting classifier
    evaluate_FraudDetectionNN_focal_loss(
        config
    )  # evaluate neural network with focal loss
