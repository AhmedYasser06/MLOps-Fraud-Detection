...
def eval_predict_with_threshold(model, x, threshold=0.5):
    ...
    y_pred = (y_pred_proba[:, 1] >= threshold).astype(
        "int"
...
def eval_precision_recall_for_different_threshold(
    y_pred_prob, y_true, title="", save_png=False, path=""
):

    precision, recall, thresholds = precision_recall_curve(
        y_score=y_pred_prob, y_true=y_true
    )
    plt.figure(figsize=(8, 6))
...
def eval_classification_report_confusion_matrix(
    y_pred, y_true, title="", save_png=False, path="", digits=5
):

    print(f"{title} Classification Report")
    print(classification_report(y_pred=y_pred, y_true=y_true, digits=digits))
    report_stats = classification_report(
        y_pred=y_pred, y_true=y_true, digits=digits, output_dict=True
    )
    ...
    labels = [
        ...
        "False Negative",
        "True Positive",
    ]  # order of confusion matrix labels
    cm = confusion_matrix(y_true=y_true, y_pred=y_pred)
    cm_flat = cm.flatten()
...
def eval_precision_recall_curve(y_pred_prob, y_true, title="", save_png=False, path=""):
    ...
    plt.plot(
        recall, precision, color="darkorange", lw=2, label="Precision-Recall curve"
    )
    plt.xlabel("Recall")
...
def eval_best_threshold(y_pred, y_true, with_repect_to="f1_score"):
    ...
    if with_repect_to == "f1_score":
        ...
        raise ValueError(
            "Invalid value for with_repect_to. Please choose 'f1_score', 'precision' or 'recall'."
        )
    ...
    print(
        ...
        optimal_threshold,
        "F1 Score:",
        f1_scores[optimal_threshold_index],
    )
...
def eval_update_model_stats(model_comparison, model_name, report_val, metric_config={}):
    ...
    if len(metric_config.keys()) == 0:
        ...
        for key, value in metric_config["pos"].items():
            if value:
                model_comparison[model_name][f"{key} positive class"] = report_val["1"][
                    key
                ]

        for key, value in metric_config["neg"].items():
            if value:
                model_comparison[model_name][f"{key} negative class"] = report_val["0"][
                    key
                ]

        if metric_config["macro_avg"]:
            model_comparison[model_name]["F1 macro avg"] = report_val["macro avg"][
                "f1-score"
            ]
...
def evaluate_model(
    ...
    if evaluation_config["train"] == True:
        ...
        eval_classification_report_confusion_matrix(
            y_train,
            y_train_pred,
            title + " train",
            save_png=save_cm_plots,
            path=eval_plots_path,
        )
        eval_precision_recall_for_different_threshold(
            y_pred_prob=y_train_pred_proba,
            y_true=y_train,
            title=title + " train",
            save_png=save_pr_rc_th_plots,
            path=eval_plots_path,
        )
        eval_precision_recall_curve(
            y_pred_prob=y_train_pred_proba,
            y_true=y_train,
            title=title + " train",
            save_png=save_roc_curve,
            path=eval_plots_path,
        )
    ...
    if evaluation_config["validation"] == True:
        ...
        eval_precision_recall_curve(
            y_pred_prob=y_val_pred_proba,
            y_true=y_val,
            title=title + " validation",
            save_png=save_roc_curve,
            path=eval_plots_path,
        )
        model_comparison = eval_update_model_stats(
            model_comparison, title, report_val, evaluation_config["metric"]
        )

        if evaluation_config["metric"]["PR_AUC"] == True:
            y_val_pred_proba = model.predict_proba(x_val)[:, 1]
            model_comparison[title]["PR AUC"] = eval_auc_precision_recall_curve(
                y_pred_prob=y_val_pred_proba, y_true=y_val
            )

        if (
            evaluation_config["optimal_threshold"] == True
        ):  # we use only training data to find optimal threshold

            y_train_pred_proba = model.predict_proba(X_train)[:, 1]
            optimal_threshold, f1_scores = eval_best_threshold(
                y_pred=y_train_pred_proba, y_true=y_train
            )

            y_val_pred = eval_predict_with_threshold(
                model=model, x=x_val, threshold=optimal_threshold
            )
            report_val = eval_classification_report_confusion_matrix(
                y_pred=y_val_pred,
                y_true=y_val,
                title=title + " val with optimal threshold",
                save_png=save_cm_plots,
                path=eval_plots_path,
            )
            model_comparison = eval_update_model_stats(
            ...
            if (
                evaluation_config["metric"]["PR_AUC"] == True
            ):  # It will same as validation (0.5 threshold)
                y_val_pred_proba = model.predict_proba(x_val)[:, 1]
                model_comparison[title + " optimal threshold"]["PR AUC"] = (
                    eval_auc_precision_recall_curve(
                        y_pred_prob=y_val_pred_proba, y_true=y_val
                    )
                )
